...
The pipeline is entirely script-based and interpreted (R, Python, Julia); no compiled standalone binary is required. All software dependencies and version numbers are listed below, organised by analysis stage, and full session details are provided in `sessionInfo.txt`.
...
See the [Key R Packages and Versions](#key-r-packages-and-versions) and [External Software & Tools](#external-software--tools) tables below for the complete dependency list with versions.
...
BiocManager::install(c("mixOmics", "ALDEx2", "phyloseq", "microbiome"))
```
...
**Typical install time:** on a normal desktop computer this may take up to ~2 hours, dominated by compiling the R/Bioconductor dependencies.

## Demo

Each figure script (`Figure_1` to `Figure_6`) is **self-contained**: it reads the processed input tables linked from this README and reproduces the corresponding manuscript figure and its underlying statistics. The processed input tables and the underlying primary data are openly available (see [Data Availability](#data-availability)).
...
```r
rmarkdown::render("Figure_2_Nature_Communications.Rmd")
```
